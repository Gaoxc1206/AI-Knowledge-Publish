---
type: index
status: public
updated: 2026-05-09 17:17
---
# AI 文献知识库

这是一个由 Zotero、Obsidian 和大模型共同维护的中文 AI 研究知识库。Zotero 保存原始文献，Obsidian 保存可阅读、可链接、可追问的研究理解。

## 当前规模

- 论文笔记：16 篇
- 概念节点：356 个
- 方法节点：260 个
- 研究地图：1 个
- 研究问题：0 个
- 更新时间：2026-05-09 17:17

## 主要入口

- [[多目标优化]]
- [[多目标分子优化]]
- [[多目标贝叶斯优化]]
- [[Expected Hypervolume Improvement]]
- [[Pareto front]]
- [[QED]]
- [[DRD2]]
- [[GFlowNets]]
- [[Bayesian optimization]]
- [[Gaussian Process]]
- [[Hypervolume Indicator]]

## 最近更新

- [[2023 - 隐式化学空间中的进化多目标分子优化 - suEvolutionaryMultiobjectiveMolecule2023]]
- [[2025 - EHVI与固定标量化在分子设计中的比较研究 - yongStudyEHVIVs2025]]
- [[面向隐式化学空间的多目标进化分子优化方法 - xiaMoleculeOptimizationMultiobjective]]
- [[面向可控生物序列设计的多目标引导离散流匹配 - chenMultiObjectiveGuidedDiscreteFlow]]
- [[目标条件GFlowNets可控多目标分子设计 - royGoalconditionedGFlowNetsControllable]]
- [[用于蛋白质序列采样与优化的帕累托最优组合能量模型 - tagasovskaParetooptimalCompositionalEnergybased]]
- [[基于大语言模型的小分子生成性质预测与优化 - bedrosianSmallMoleculeOptimization]]
- [[2025 - 扩散推理时多目标生成的分布式多目标黑箱优化 - tanDistributionalMultiobjectiveBlackbox2025]]
- [[2025 - 可控多属性多目标分子优化的大语言模型 - deyLargeLanguageModels2025]]
- [[2025 - AReUReDi多目标引导离散流退火校正更新 - chenAReUReDiAnnealedRectified2025]]

## 论文库

### 多模态多目标

- 论文数量：2
- 路径：`wiki/papers/多模态多目标/`

- [[2025 - 用于多峰多目标优化的自适应适应度共享进化策略 - ahrariEvolutionStrategyAdaptive2025]]
- [[2026 - 自适应收敛指标与邻域模糊拥挤距离的多峰多目标优化 - dangAdaptiveConvergenceIndicator2026]]

### 多目标分子优化

- 论文数量：14
- 路径：`wiki/papers/多目标分子优化/`

- [[2023 - 隐式化学空间中的进化多目标分子优化 - suEvolutionaryMultiobjectiveMolecule2023]]
- [[2025 - AReUReDi多目标引导离散流退火校正更新 - chenAReUReDiAnnealedRectified2025]]
- [[2025 - EHVI与固定标量化在分子设计中的比较研究 - yongStudyEHVIVs2025]]
- [[2025 - 可控多属性多目标分子优化的大语言模型 - deyLargeLanguageModels2025]]
- [[2025 - 扩散推理时多目标生成的分布式多目标黑箱优化 - tanDistributionalMultiobjectiveBlackbox2025]]
- [[2025 - 语言模型的帕累托多目标对齐 - heParetoMultiObjectiveAlignment2025]]
- [[MOLLM借助专家优化的分子设计多目标大语言模型 - ranMOLLMMultiObjectiveLarge]]
- [[分子设计中 EHVI 与固定标量化方法比较 - yongStudyEHVIVs]]
- [[基于大语言模型的小分子生成性质预测与优化 - bedrosianSmallMoleculeOptimization]]
- [[基于数据物理双驱动生成框架的多目标荧光分子设计 - liMultiobjectiveFluorescentMolecule]]
- [[用于蛋白质序列采样与优化的帕累托最优组合能量模型 - tagasovskaParetooptimalCompositionalEnergybased]]
- [[目标条件GFlowNets可控多目标分子设计 - royGoalconditionedGFlowNetsControllable]]
- [[面向可控生物序列设计的多目标引导离散流匹配 - chenMultiObjectiveGuidedDiscreteFlow]]
- [[面向隐式化学空间的多目标进化分子优化方法 - xiaMoleculeOptimizationMultiobjective]]

## 知识节点

- `wiki/concepts/`：概念节点，回答“这个概念是什么，以及它在多篇论文中如何被使用”。
- `wiki/methods/`：方法节点，回答“这个方法如何工作、解决什么问题、适合什么场景、有哪些局限”。
- `wiki/maps/`：研究地图和主题路线，用来组织一个方向。
- `wiki/questions/`：研究问题、综合回答和未来想法。

## 使用原则

- Zotero 是 source of truth，Obsidian 是 synthesized knowledge。
- `raw/` 保存原始 PDF、关键图和同步资源，尽量不要手工改。
- `wiki/papers/` 记录单篇论文理解。
- `wiki/concepts/` 和 `wiki/methods/` 承担跨论文综合。
- 不要为了图谱好看而过度链接。
